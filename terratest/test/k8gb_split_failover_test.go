...
func TestK8gbSplitFailoverExample(t *testing.T) {
	...
	t.Run("Each cluster resolves its own set of IP addresses", func(t *testing.T) {
		beforeFailoverResponseCluster1, err := utils.WaitForLocalGSLB(t, settings.DNSServer, settings.Port, "terratest-failover-split."+settings.DNSZone, expectedIPsCluster1)
		require.NoError(t, err)

		assert.Equal(t, beforeFailoverResponseCluster1, expectedIPsCluster1)

		beforeFailoverResponseCluster2, err := utils.WaitForLocalGSLB(t, settings.DNSServer, settings.Port, "terratest-failover-split."+settings.DNSZone, expectedIPsCluster2)
		require.NoError(t, err)
	...
	t.Run("Cluster 1 failovers to Cluster 2", func(t *testing.T) {
		afterFailoverResponse, err := utils.WaitForLocalGSLB(t, settings.DNSServer, settings.Port, "terratest-failover-split."+settings.DNSZone, expectedIPsCluster2)
		require.NoError(t, err)
	...
	t.Run("Cluster 1 returns own entries again", func(t *testing.T) {
		afterFailoverResponse, err := utils.WaitForLocalGSLB(t, settings.DNSServer, settings.Port, "terratest-failover-split."+settings.DNSZone, expectedIPsCluster1)
		require.NoError(t, err)
